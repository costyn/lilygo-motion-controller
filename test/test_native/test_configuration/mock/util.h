#pragma once

#include <string>

// Log levels (must match real util.h)
typedef enum {
    LOG_LEVEL_ERROR = 0,
    LOG_LEVEL_WARN = 1,
    LOG_LEVEL_INFO = 2,
    LOG_LEVEL_DEBUG = 3
} log_level_t;

// Mock log function (no-op for tests)
inline void logPrint(log_level_t level, const char* function, const char* format, ...) {
    // No-op for tests
}

// Mock other util functions
inline std::string timeToString() { return "00:00:00.000"; }
inline float fmap(float x, float a, float b, float c, float d) { return 0.0f; }

// Logging macros
#define LOG_ERROR(fmt, ...) logPrint(LOG_LEVEL_ERROR, __func__, fmt, ##__VA_ARGS__)
#define LOG_WARN(fmt, ...)  logPrint(LOG_LEVEL_WARN, __func__, fmt, ##__VA_ARGS__)
#define LOG_INFO(fmt, ...)  logPrint(LOG_LEVEL_INFO, __func__, fmt, ##__VA_ARGS__)
#define LOG_DEBUG(fmt, ...) logPrint(LOG_LEVEL_DEBUG, __func__, fmt, ##__VA_ARGS__)
